...
/**
 * GET /api/strings-jobs/[jobId]/resume
 */
export async function GET(_request: NextRequest, { params }: RouteParams) {
  ...
  if (!['interrupted', 'failed'].includes(job.status)) {
    if (job.status === 'completed') return sseErrorResponse('Job already completed', 400)
  ...
  const claimed = claimStringsJobForProcessing(jobId, ['interrupted', 'failed'])
  if (!claimed) {
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        let results: XCStringsFile = job.results
          ? cloneXCStringsFile(job.results)
          : cloneXCStringsFile(sourceContent)
        ...
        sendEvent({
          ...
          completedLocales: completedLocalesCount,
          progress: Math.round(
        ...
        for (const locale of targetLocales) {
          if (localeResults[locale]?.status === 'completed') {
            sendEvent({
              ...
              translatedCount: localeResults[locale].translated_count || 0,
            })
          }
        }

        const localesToProcess = targetLocales.filter(
          (locale) => localeResults[locale]?.status !== 'completed'
        )
        ...
        const processLocale = async (
          ...
          const localLocaleResult: StringsLocaleResult =
            localeResults[locale]?.status === 'failed'
              ? {
                  status: 'processing',
                  batches: localeResults[locale].batches || {},
                  translated_count: 0,
                }
              : { status: 'processing', batches: {}, translated_count: 0 }
          ...
          for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
            ...
            if (localLocaleResult.batches[String(batchIndex)] === 'completed') {
              const batch = batches[batchIndex]
              for (const entry of batch) {
                const existingTranslation =
                  results.strings[entry.key]?.localizations?.[locale]?.stringUnit
                    ?.value
                if (existingTranslation) {
                  localeTranslations[entry.key] = existingTranslation
                }
              }
              continue
            }
        ...
        console.error('Error in strings resume stream:', error)
        updateStringsJob(jobId, {
          status: 'interrupted',
